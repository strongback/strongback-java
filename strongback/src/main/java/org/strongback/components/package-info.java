 ...
 * <ul>
 * <li>{@link AngleSensor} - A sensor that returns instantaneous angles measured in degrees. The sensor can return absolute
 ...
 * <li>{@link CurrentSensor} - A sensor that returns the instantaneous and absolute current measured in Amps.</li>
 * <li>{@link VoltageSensor} - A sensor that returns the instantaneous and absolute voltage measured in Volts.</li>
 * <li>{@link TemperatureSensor} - A sensor that returns the instantaneous and absolute temperature measured in degrees Celcius.</li>
 * <li>{@link DistanceSensor} - A sensor that returns the instantaneous distance measured in inches or feet from a nearby
 ...
 * <li>{@link Switch} - A switch represents a device that has an active state when it is triggered and an inactive state when
 * it isn't. It can represent buttons, limit switches, Reed switches, Hall effect sensors, etc.</li>
 * <li>{@link Fuse} - A device that acts like a {@link Switch} but that can be explicitly triggered and optionally reset.</li>
 * <li>{@link SpeedSensor} - A sensor that returns the instantaneous speed. SpeedSensor may be useful on its own,
 * but it is intended primarily as a base interface for {@link Motor}</li>
 * <li>{@link SpeedController} - A device whose speed is able to be controlled. SpeedController may be useful on its own,
 * but it is intended primarily as a base interface for {@link Motor}.</li>
 * <li>{@link Accelerometer} - A sensor that provides the acceleration along a single axis.</li>
 * <li>{@link Relay} - A device that can be turned on and off.</li>
 * <li>{@link Solenoid} - A device that can be is a device that can be extended and retracted. Although similar to a
 * {@link Relay}, pneumatic solenoids are common enough in FRC to distinguish as a separate low-level component.</li>
 * </ul>
 ...
 * <ul>
 * <li>{@link Motor} - A device that is both a {@link SpeedSensor} and {@link SpeedController}. Quite obviously, it can
 * represent a combination of an electric motor (e.g., CIM) and electronic speed controller (e.g., Talon).</li>
 * <li>{@link LimitedMotor} - A {@link Motor} that is constrained to move between a maximum position and minimum position.</li>
 * <li>{@link TalonSRX} - A motor controlled by a Talon SRX with a current sensor and position (angle) sensor, and that is
 * optionally constrained to move between a maximum position and minimum position.</li>
 * <li>{@link Compass} - An {@link AngleSensor angle sensor} that provides heading information in addition to angle.</li>
 * <li>{@link Gyroscope} - A gyroscope is a device that measures angular velocity (in degrees per second) about a single axis.
 ...
 * {@link Compass} (which extends {@link AngleSensor}).
 * <li>{@link TwoAxisAccelerometer} - A combination of two single-axis {@link Accelerometer}s. It can be used to represent
 * a physical 2- or 3-axis accelerometer, such as the ADXL193 (which may no longer be supported by WPILib).</li>
 * <li>{@link ThreeAxisAccelerometer} - A combination of two single-axis {@link Accelerometer}s. It can be used to represent
 * a physical 3-axis accelerometer, such as the ADXL345 or built-in accelerometer on the RoboRIO.</li>
 * <li>{@link PowerPanel} - An abstraction of the sensors on the Power Distribution Panel.</li>
 * <li>{@link SolenoidWithPosition} - A {@link Solenoid} that can determine its position. Typically this would be used to
 ...
 * <ul>
 * <li>{@link Stoppable} - Something that can be stopped.</li>
 * <li>{@link Zeroable} - Something that can be zeroed.</li>
 * </ul>
